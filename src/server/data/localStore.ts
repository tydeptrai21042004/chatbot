...
export function deleteUser(userId:string){
  const db=readJson(usersFile(),emptyUsers()); const target=db.users.find(u=>u.id===userId&&u.role==='student'); if(!target)return false;
  db.users=db.users.filter(u=>u.id!==userId); atomicWrite(usersFile(),db); return true;
}
