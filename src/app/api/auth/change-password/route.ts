...
export async function POST(req:NextRequest){
  ...
  const user=await findUserById(identity.id); if(!user)return NextResponse.json({ok:false,error:"Không tìm thấy tài khoản"},{status:404});
  const updated=await updateUserPassword(user.id,hashPassword(parsed.data.password),false);
